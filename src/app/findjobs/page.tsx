...
export default function JobFilterBar() {
	...
	useEffect(() => {
		...
		setFilters((prev) => ({
			...prev,
			category:
				category !== "All" ? decodeURIComponent(category) : "All",
			city: city !== "All" ? decodeURIComponent(city) : "All",
			company:
				company !== "All" ? decodeURIComponent(company) : "All",
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				if (allJobs.length === 0) {
					setError((prev) =>
						prev ? `${prev} No jobs found.` : "No jobs found."
					)
				}
			} catch (err) {
	...
	useEffect(() => {
		console.log(
			"Checking auto-select. Current selectedJob:",
			selectedJob?.id
		)

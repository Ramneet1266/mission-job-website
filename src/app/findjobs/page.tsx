...
export default function JobFilterBar() {
  ...
  useEffect(() => {
    if (window.location.hash.includes('googtrans')) {
      router.replace(`/findjobs?${searchParams.toString()}`, { scroll: false })
  ...
  useEffect(() => {
    ...
    setFilters((prev) => ({
      ...prev,
      category: category && decodeURIComponent(category) !== "null" ? decodeURIComponent(category) : "All",
      city: city && decodeURIComponent(city) !== "null" ? decodeURIComponent(city) : "All",
      company: company && decodeURIComponent(company) !== "null" ? decodeURIComponent(company) : "All",
      tags: tag && decodeURIComponent(tag) !== "null" ? [decodeURIComponent(tag)] : [],
    }))
  }, [searchParams])
  ...
  const handleRemoveInput = (key: keyof typeof searchQuery) => {
    ...
    router.replace(`/findjobs?${newParams.toString()}`, { scroll: false })
  }
    ...
    const fetchData = async () => {
      try {
        ...
        for (const category of categoriesSnapshot.docs) {
          const postingsSnapshot = await getDocs(collection(db, "categories", category.id, "posting"))
        ...
        if (allJobs.length === 0) {
          setError((prev) => (prev ? `${prev} No jobs found.` : "No jobs found."))
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const tagsFromJobs = Array.from(new Set(filteredJobs.flatMap((job) => job.tags)))
    const tagFromUrl = searchParams.get("tags")
    const tags = tagFromUrl ? Array.from(new Set([...tagsFromJobs, decodeURIComponent(tagFromUrl)])) : tagsFromJobs
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    const matchesSalary =
      ...
      (filters.salary === "Rs 0 - 20000"
        ? Number.isNaN(parseInt(job.salary.replace("Rs ", ""))) ? false : parseInt(job.salary.replace("Rs ", "")) <= 20000
        : filters.salary === "Rs 20000 - 40000"
        ? Number.isNaN(parseInt(job.salary.replace("Rs ", ""))) ? false : parseInt(job.salary.replace("Rs ", "")) > 20000 && parseInt(job.salary.replace("Rs ", "")) <= 40000
        : Number.isNaN(parseInt(job.salary.replace("Rs ", ""))) ? false : parseInt(job.salary.replace("Rs ", "")) > 40000)
  ...
  const toggleFilterOption = (filter: FilterKey, value: string) => {
    if (filter === "tags") {
      ...
      const newParams = new URLSearchParams(searchParams.toString())
      const newTags = filters.tags.includes(value) ? filters.tags.filter((t) => t !== value) : [...filters.tags, value]
      if (newTags.length > 0) {
  ...
  return (
    <ErrorBoundary>
      <div className="mt-28 min-h-screen bg-gradient-to-br from-blue-50 to-white pb-16">
        {/* FILTER BAR */}
        <div className="border-b-2 border-blue-200 py-4 flex justify-center bg-white shadow-sm sticky top-24 z-60 rounded-b-md notranslate" translate="no">
              ...
              <div key={filter.label} className="relative">
                <button
                  ...
                  className={`flex items-center gap-1 border-2 border-blue-300 px-3 py-1 rounded-full transition-all duration-200 ${
                    activeFilter === filter.label ? "bg-blue-100 text-blue-800" : "bg-white text-gray-800 hover:bg-blue-50"
        ...
        <div className="py-3 px-5 bg-white shadow-md rounded-lg flex gap-2 flex-wrap max-h-24 overflow-y-auto notranslate" translate="no">
          {searchQuery.query && (
            <Tag label={`Query: ${searchQuery.query}`} onRemove={() => handleRemoveInput("query")} />
          )}
          {searchQuery.location && (
            <Tag label={`Location: ${searchQuery.location}`} onRemove={() => handleRemoveInput("location")} />
          )}
          {searchQuery.experience !== "Experience" && (
            <Tag label={`Experience: ${searchQuery.experience}`} onRemove={() => handleRemoveInput("experience")} />
          )}
          {filters.category !== "All" && (
            <Tag label={`Category: ${filters.category}`} onRemove={() => handleRemoveFilter("category")} />
          )}
          {filters.tags.map((tag) => (
            <Tag key={tag} label={`Tag: ${tag}`} onRemove={() => handleRemoveFilter("tags", tag)} />
          ))}
          {filters.city !== "All" && (
            <Tag label={`City: ${filters.city}`} onRemove={() => handleRemoveFilter("city")} />
          )}
          {filters.state !== "All" && (
            <Tag label={`State: ${filters.state}`} onRemove={() => handleRemoveFilter("state")} />
          )}
          {filters.salary !== "All" && (
            <Tag label={`Salary: ${filters.salary}`} onRemove={() => handleRemoveFilter("salary")} />
          )}
          {filters.company !== "All" && (
            <Tag label={`Company: ${filters.company}`} onRemove={() => handleRemoveFilter("company")} />
          )}
        </div>
        ...
        <div className="flex h-full">
          ...
          <div className="w-1/2 border-r-2 border-blue-200 overflow-y-auto p-4 space-y-3 bg-white rounded-l-md shadow-md">
            {loading ? (
              <p className="text-center text-blue-600 font-medium animate-pulse">Loading...</p>
            ) : error ? (
              <p className="text-center text-red-600 font-semibold">{error}</p>
            ) : filteredJobs.length === 0 ? (
              <p className="text-center text-gray-500 italic">No jobs match your filters.</p>
            ) : (
                ...
                >
                  <div
                    className={`p-3 rounded-md border border-gray-200 bg-white hover:bg-blue-50 transition-all duration-200 cursor-pointer shadow-sm ${
                      selectedJob?.id === job.id ? "border-blue-500 bg-blue-100 ring-1 ring-blue-300" : ""
                    }`}
                  >
                    <h3 className="text-md font-semibold text-blue-800 mb-1">{job.jobTitle}</h3>
                    <p className="text-sm text-gray-700 mb-1">{job.jobCompany}</p>
            ...
            {selectedJob ? (
              user ? (
                <div className="space-y-3">
                  <h2 className="text-xl font-bold text-blue-900">{selectedJob.jobTitle}</h2>
                  <p className="text-md text-gray-700">{selectedJob.jobCompany}</p>
                  <img
                  ...
                  <Section title="Location" content={`${selectedJob.city}, ${selectedJob.state}`} />
                  <Section title="Address" content={selectedJob.address || "Not specified"} />
                  <Section title="Category" content={selectedJob.category} />
                  <Section title="Contact" content={`${selectedJob.contactEmail} | ${selectedJob.contactNumber}`} />
                  <Section
                    title="Job Description"
                    content={selectedJob.jobDescription || "No description available"}
                  />
                  <Section
                  ...
                  <button className="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all duration-200 shadow-md">
                    Quick Apply
                  </button>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center h-full space-y-4">
                  <p className="text-center text-blue-600 text-md font-medium">Please sign in to view job details</p>
                  <button
              ...
              <p className="text-center text-blue-600 text-md font-medium">Select a job to view details!</p>
            )}
          </div>
        </div>
      </div>
